...
eval(loadDependency(['Db','List','Attack','Tk','Quest','Ability']));
...
Db.query.ability = function(info){	
	return Ability.objectVersion(info);
}	
...
Db.query.highscore = function(d,cb){
	...
	if(!List.main[d.key] || !List.main[d.key].quest[d.id.split('-')[0]]) return ERROR(3,'bad highscore',d.id);
	
	Quest.highscore.fetch(d.id,function(res){
		for(var i in res){	//test if already there
			if(List.nameToKey[res[i].username] === d.key){	//already in list, no need to add at end, call cb straight away
				cb({
					info:res,
					db:'highscore',
					id:d.id,
		...
		Quest.highscore.fetchRank(d.key,d.id,function(res2){	//other fetch self too
			res.push(res2);
			cb({
				info:res,
				db:'highscore',
...
Db.query = function(db,id,update){	//update forces the query
	if(!id || !db) return;
