...
eval(loadDependency(['Db','List','Tk','Init','Collision'],['Anim']));
...
Init.db.anim.creation = function(anim){
	...
	anim = Tk.useTemplate(Init.db.anim.creation.template(),anim,false);		
	anim.img = Tk.newImage(anim.src);
...
Anim.creation = function(d){	//server
	...
	var anim = Tk.useTemplate(Anim.template(),d,true);
	
	if(typeof anim.target === 'string'){
		if(!List.map[List.all[anim.target].map]) return ERROR(4,'no map',List.all[anim.target].map);
		List.map[List.all[anim.target].map].list.anim[anim.id] = anim;
	} else {
		if(!List.map[anim.target.map]) return ERROR(4,'no map',anim.target.map);
		List.map[anim.target.map].list.anim[anim.id] = anim;
	}
	return anim.id;
}
Anim.template = function(){
	return {
		sizeMod:1,
		name:'scratch',
		target:{x:1,y:1,map:'QfirstTown-main@MAIN'},
...
if(!SERVER){	//client
	...
	Anim.creation = function(a){
		a = Tk.useTemplate(Anim.template(),a,true);
		if(typeof a.target === 'string'){
			a.target = a.target === player.name ? player : List.all[a.target];	//player.name is publicId
			if(!a.target) return;
		}
