...
eval(loadDependency(['Db','List','Actor','Init','Chat','Main','requireDb','Sprite'],['Command']));

if(SERVER) var db = requireDb();
var Contribution = exports.Contribution = {};

var chat = function(key,text){
	Chat.add(key,{type:'contribution',text:text});
}
...
Contribution.globalMessage.approve = function(num){	//no clue if work
	...
	if(List.nameToKey[a.username]){
		var key = List.nameToKey[a.username];
	...
	db.findOne('main',{username:a.username},{},function(err,res){
		...
		db.update('main',{username:a.username},res,db.err);
	}
}
...
Contribution.globalMessage.refuse = function(num,text){
	...
	if(List.nameToKey[a.username]){
		chat(List.nameToKey[a.username],'Your global message has been refused.' + text);
		List.main[List.nameToKey[a.username]].contribution.reward.globalMessage.status = 'Refused' + text;4
	}
	db.findOne('main',{username:a.username},{},function(err,res){
		if(err) throw err; if(!res) return;
...
Contribution.purchase = function(key,type,param){
	...
	if(type === 'player'){
		...
		for(var i in list) if(!Db.sprite[list[i]] || !Db.sprite[list[i]].player) return chat(key,'Invalid sprite: ' + list[i] + '.');
		
		c.point.usable -= cost;
		c.point.used += cost;
		c.reward.history.push({date:Date.now(),type:type,cost:cost,param:param});
		c.reward.player = {name:param,date:Date.now()};
		List.all[key].sprite.normal = param;
...
Contribution.select = function(key,type,param){
	...
	if(type === 'chatText'){
		...
		main.social.customChat = Contribution.getChatCustom(key);
...
Contribution.reset = function(key,type){
	...
	if(type === 'player'){
		List.all[key].sprite.normal = 'mace';
		c.reward.player = {name:'mace',date:Date.now()};
		Sprite.change(List.all[key],{name:'normal'});
	}	
}

Contribution.change = function(key,account,name){
	var p = List.main[key].contribution.point;
	if(!p[account]) return;
	p[account].username = name;
	List.main[key].flag.contribution = 1;
...
Init.contribution = function(first){	//client, set HTML
	Init.contribution.spend(first);
	Init.contribution.earn(first);
}

Init.contribution.earn = function(first){
	...
	$("#contributionQuestHistory")[0].innerHTML = Init.contribution.earn.history(p.quest);	//array
	$("#contributionTwitterHistory")[0].innerHTML = Init.contribution.earn.history(p.twitter);	
	$("#contributionTwitchHistory")[0].innerHTML = Init.contribution.earn.history(p.twitch);
	$("#contributionRedditHistory")[0].innerHTML = Init.contribution.earn.history(p.reddit);
	$("#contributionYoutubeHistory")[0].innerHTML = Init.contribution.earn.history(p.youtube);
...
Init.contribution.earn.history = function(info){
...
Init.contribution.spend = function(first){
	...
	//Symbol
	var list = Contribution.getChatCustom.list[0];
	...
	//Chat
	var list = Contribution.getChatCustom.list[1];
	for(var i in list){
	...
	*/
	...
	if(first){
		var str = {};
		for(var i in Init.db.sprite.player){	
			str[i] = '<option value="NONE">None</option>';
			for(var j in Init.db.sprite.player[i]){
				var s = Init.db.sprite.player[i][j];
...
Contribution.getChatCustom = function(key){
	var main = List.main[key];
	var str = '000';
	...
	for(var i in main.contribution.reward.symbol){
		if(main.contribution.reward.symbol[i] === 2){
			str = str.set(0,'' + Contribution.getChatCustom.list[0].indexOf(i));
		}
	}
	for(var i in main.contribution.reward.chatText){
		if(main.contribution.reward.chatText[i] === 2){
			str = str.set(1,'' + Contribution.getChatCustom.list[1].indexOf(i));
		}
	}
...
Contribution.getChatCustom.list = [
...
Contribution.purchase = function(type,param){
	Command.send('reward,purchase,' + type + ',' + param);
}

Contribution.select = function(type,param){
	Command.send('reward,select,' + type + ',' + param);
}

Contribution.reset = function(type){
	Command.send('reward,reset,' + type);
}

Contribution.change = function(account,name){
	Command.send('reward,change,' + account + ',' + name);
}
Contribution.updateSocialMedia = function(account){
	Command.send('reward,updateSocialMedia,' + account);	
}
//{
}
