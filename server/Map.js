...
eval(loadDependency(['List','Actor','Loop','Activelist','Collision'],['Map']));
/*
Db.map loaded.
Map model created
Quest loaded
...
Map.loop = function(map){
	//Time Out Instance
	if(Loop.interval(10*1000/25)){		//each 10 sec, incase big bug...
		if(map.list.player.$empty()){
			Map.remove(map);
			return;
		}	
	}
