...
Draw.entity.drop = function(key){
	...
	if(!server){
		...
		for(var i in dropList){
			...
			ctx.drawImage(itemSheet,slot.x,slot.y,ITEM,ITEM,numX,numY,32,32);	
		}
	}
}
//}
...
Draw.window.main = function(key,title){ ctxrestore();	
	...
	if(!server){
		...
		ctx.drawImage(imgFrame.window,0,0,imgFrame.window.width,imgFrame.window.height,sx-30,sy-25,w+75,h+25);
		ctx.globalAlpha = 1;
...
Draw.window.stat.hover = function(hover,type){ ctxrestore();
	...
	//Frame
	var ctx = ctxList.pop;
	ctx.drawImage(imgFrame.postit,0,0,imgFrame.postit.width,imgFrame.postit.height,numX,numY-15,400,500);
	...
	if(dia){
		...
		if(!server){
			...
			if(dia.face){
				...
				ctx.drawImage(npcFaceSheet,0,0,96,96,facesX,sy+facesY,96,96);
				ctx.font="20px Fixedsys";
				ctx.textAlign = 'center';
